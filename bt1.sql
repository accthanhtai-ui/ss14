CREATE DATABASE RikkeiClinicDBb;
USE RikkeiClinicDBb;
...
delimiter ;
CALL PayHospitalFee(1,200000); -- khi chạy em thấy nó lỗi ngay là ghi lỗi hệ thống ở dưới và đây là phần A của em ở bt 1
-- tiến hành em kiểm tra dữ liệu ở chổ bản ví 
SELECT * FROM Wallets; -- còn 300 000 vnd
SELECT * FROM Patient_Invoices; -- kiểm tra bảng nợ là nó vẫn còn chứng tỏ đó là lỗi của Atomicity tính nguyên tử tiền trừ nhưng nợ chưa hết
-- tiến hành phần B 
-- xóa procedure PayHospitalFee nếu có tồn tại
drop procedure  if exists PayHospitalFee;
-- tạo procedure mới có Transaction
-- update trả về tiền cũ để làm phần B
UPDATE Wallets
SET balance = 500000
WHERE patient_id = 1;
delimiter //
